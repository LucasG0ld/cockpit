...
2. **IMPLEMENT** `PATCH /memberships/:id/status` acceptant `Disabled` ou `Active`.
3. **HANDLE** désactivation : check CSM (clients actifs) & Closer (transfert onboarding) avant commit.
4. **SET** `status`, `disabledAt`, et `role = Temporaire` lors réactivation; forcer Admin à reconfigurer le rôle via réponse.
5. **INVALIDATE** sessions via Clerk et consigner les audit logs.
6. **TEST** e2e couvrant désactivation simple, blocage CSM sans réassignation, réactivation -> rôle Temporaire.
