...
---
# Clé utilisée par l'agent pour router vers le bon cycle d'exécution.
# Valeurs possibles : 'development', 'migration', 'policy' (exigence transversale non exécutable).
task_type: '[development|migration|policy]'

# Nom de la migration (UNIQUEMENT si task_type est 'migration').
# Sera utilisé pour la commande `prisma migrate dev --name "..."`.
# Exemple : 'add-user-profile-table'
...
- [ ] **Validation des Entrées :** Toutes les entrées externes sont validées.
- [ ] **Gestion des Secrets :** Aucun secret n'est codé en dur.
- [ ] **Performance :** Aucun goulot d'étranglement de performance évident (ex: requêtes N+1) n'a été introduit.
