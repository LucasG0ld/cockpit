...
7.  **"Gestion de Dépendances par Changement de Contexte" :** Si, au cours d'une tâche, tu es bloqué par une dépendance inachevée, tu dois lancer un **protocole de validation de dépendance** en deux étapes :
    ...
    2.  **Prise de Décision Basée sur les Faits :**
        *   **Si les deux tests (A et B) réussissent :** La dépendance est **validée**. Ton devoir est de **STOPPER**, de **signaler** le blocage en utilisant le template `@.windsurf/rules/templates/anomaly_report.template.md`, et de proposer d'initier le `Workflow F`.
...
8.  **"Doctrine de Recherche de Fichiers par Typologie" :** Si tu ne trouves pas un fichier, tu dois d'abord identifier son **type** et appliquer la stratégie d'enquête correspondante :
    *   **1. Si c'est un FICHIER DE RÈGLES (`.md`) :** L'absence de ce fichier est une **anomalie critique** de notre système. Tu dois **STOPPER** et escalader immédiatement.
    *   **2. Si c'est un ARTEFACT DE PROJET (`_plan.md`, `PRD`, `ADR`, etc.) :** C'est un prérequis essentiel. L'absence est une **défaillance de processus**. Applique l'algorithme de diagnostic suivant :
        *   a. **Analyse de la Cause :** Vérifie si, selon ton plan (`_plan.md` ou `task.md`), tu étais censé créer ce fichier. Si oui, signale l'échec de la création.
        *   b. **Tentative de Localisation :** Si non, charge `@.windsurf/rules/folder-structure.md` et tente de trouver le fichier à un autre emplacement.
